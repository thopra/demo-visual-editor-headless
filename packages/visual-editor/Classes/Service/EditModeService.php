...
final readonly class EditModeService
{
    ...
    public function init(ServerRequestInterface $request): void
    {
        ...
        foreach ($this->getStyleResources() as $key => $stylesheet) {
            $this->assetCollector->addStyleSheet($key, $stylesheet);
        }

        foreach ($this->getJavaScriptModules() as $jsModule) {
            $this->assetCollector->addJavaScriptModule($jsModule);
        }
        ...
        if (!$this->assetCollector->hasInlineJavaScript('veLangInfo')) {
            ...
            $veInfo = $this->getPageInfo($request);
...
if (window.parent === window && window.veInfo) {
  ...
  if (backendEditUrl) {
    ...
    public function getStyleResources(): array
    {
        return [
    ...
    public function getJavaScriptModules(): array
    {
        ...
        if ($this->typo3Version->getMajorVersion() >= 14) {
            $jsModules[] = '@typo3/backend/element/contextual-record-edit-trigger.js';
        }

        return $jsModules;
    }

    public function getPageInfo(ServerRequestInterface $request): array
    {
        if (!$this->isEditMode($request)) {
            return [];
        }
    ...
    {
        foreach ($this->getLanguageLabels() as $key => $value) {
